...
class DifferentialEvolutionOptimization:
    def __init__(self, simulation : Simulation, kwargs : RoutingData, de_model : DEModel):
        ...
        self.kwargs = {key: kwargs[key] for key in ["pn","qb","sim","loss"] }
    ...
    def optim(self): 
        ...
        #raise Exception(self.kwargs, self.kwargs_length)
                        
        model = differential_evolution(func = self.de_func, bounds = pars_bounds, updating='immediate')
